...
if ( basename(KissMetrics::array_get($_SERVER,'SCRIPT_NAME')) == basename(__FILE__) )
{
  if (!KissMetrics::array_get($argv,1))
    error_log("At least one argument required. km.php <km_key> [<log_dir>]");

  KissMetrics::init(KissMetrics::array_get($argv,1), array('log_dir' => KissMetrics::array_get($argv,2,KissMetrics::$log_dir,true), 'host' => KissMetrics::array_get($argv,3,KissMetrics::$host,true) ));
  KissMetrics::send_logged_queries();
}
